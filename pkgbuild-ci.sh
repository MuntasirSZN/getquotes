...
mapfile -t SOURCES < <(sed -n '/^source=(/,/^)/ { /^source=(/d; /^)/d; /^[[:space:]]*#/d; s/^[[:space:]]*"\([^"]*\)".*$/\1/; p }' PKGBUILD)
...
mapfile -t OLD_SUMS_QUOTED < <(sed -n '/^sha256sums=(/,/^)/ { /^sha256sums=(/d; /^)/d; /^[[:space:]]*#/d; s/^[[:space:]]*"\([^"]*\)".*$/"\1"/; p }' PKGBUILD)
